...
class EmailService {
    ...
    async sendTemplateEmail(customerEmail, orderID, paymentData = {}) {
        if (paymentData.isGift) {
            ...
            await this.sendGiftConfirmation(
                customerEmail,
            ...
            const mailOptions = {
                ...
                html: `
                    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                        <h1 style="color: #0984E3;">Thank You!</h1>
    ...
    async sendGiftEmail(recipientEmail, senderName, giftMessage, orderID) {
        console.log('Preparing to send gift email to:', recipientEmail);
        const mailOptions = {
            from: process.env.EMAIL_USER,
            to: recipientEmail,
            subject: `${senderName} sent you a special gift! 🎁`,
            html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    <h1 style="color: #0984E3;">You've Received a Gift! 🎁</h1>
                    
                    <div style="background-color: #f5f6fa; padding: 20px; border-radius: 10px; margin: 20px 0;">
                        <h2 style="color: #2D3436;">A Note from ${senderName}</h2>
                        <p style="font-style: italic; padding: 15px; background: white; border-radius: 8px;">
                            "${giftMessage}"
                        </p>
                    </div>

                    <p>${senderName} has gifted you The Focus Zone template - a powerful Notion template for peak productivity!</p>
    ...
    async sendGiftConfirmation(customerEmail, recipientName, orderID) {
        console.log('Preparing to send gift confirmation email to:', customerEmail);
        const mailOptions = {
            from: process.env.EMAIL_USER,
            to: customerEmail,
            ...
            html: `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                    <h1 style="color: #0984E3;">Your Gift to ${recipientName} has been sent! 🎉</h1>
                    
                    <p>Thank you for gifting The Focus Zone template to ${recipientName}!</p>
        ...
        try {
            await this.transporter.sendMail(mailOptions);
            console.log('Gift confirmation email sent successfully');
            return true;
        } catch (error) {
            console.error('Error sending gift confirmation email:', error);
            throw error;
        }
    }
}
